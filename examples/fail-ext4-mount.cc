...
class BPFKIClient final: public bpfki::BPFKIService::Service {
 public:
  ...
  void FailMMOrBIO(uint32_t pid, uint32_t tid, float pct) {
    ...
    req.set_ftype(bpfki::FailKernRequest_FAILTYPE_SLAB);
    auto frame0 = req.add_callchain();
    frame0->set_funcname("ext4_mount");
    auto frame1 = req.add_callchain();
    frame1->set_funcname("mount_subtree");
